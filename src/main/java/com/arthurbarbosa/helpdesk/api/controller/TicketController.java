package com.arthurbarbosa.helpdesk.api.controller;
...
import com.arthurbarbosa.helpdesk.api.dto.Summary;
...
import com.arthurbarbosa.helpdesk.api.enums.StatusEnum;
import com.arthurbarbosa.helpdesk.api.response.Response;
import com.arthurbarbosa.helpdesk.api.security.jwt.JwtTokenUtil;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> createOrUpdate(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			...
			ticket.setNumber(generatedNumber());
	...
	public ResponseEntity<Response<Ticket>> findById(@PathVariable("id") String id) {
		...
		Iterable<ChangesStatus> changesCurrent =  ticketService.listChangeStatus(ticket.getId());
    ...
    public  ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, 
		...
		if(number > 0) {
			...
			if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				...
				tickets = ticketService.findByParametersByAndCurrentUser(page, count, title, status, priority, userRequest.getId());
			}
		}
		response.setData(tickets);
		return ResponseEntity.ok(response);
